mod interface_non_rolling_hash;
mod interface;
mod modular;
mod non_rolling_hash;
mod one_zero;
mod optimal_substring;
mod rolling_hash;

use pyo3::prelude::*;
use rolling_hash::{RollingHash, DEFAULT_MOD_U64};

use crate::rolling_hash::DEFAULT_BASE;
...
mod tests {
    ...
    fn _test_rolling_hash_str_once<TH, TD>(
        text: &Vec<TD>,
        window_size: usize,
        base: TH,
        modulus: TH,
        ...
        TD: Copy + Into<BigInt>,
    {
        ...
        let rolling_results: Vec<_> =
            rolling_hash::RollingHash::new(&text, window_size, base, modulus)
                .unwrap()
                .collect();
    ...
    fn test_most_common_substring() {
        ...
        let len: usize = 50;
        let rolling =
            optimal_substring::most_common_of_len::<TD, usize, usize, TH, RollingHash<TD, TH>>(
                &resume,
                &|_, _, _| true,
                true,
                len,
...
/// rolling_hash_py(text: str, window_size: int, base: int | None = None, modulus: int | None = None) -> list[tuple[int, int]]
/// --
///
/// Calculate rolling hash values for all windows in text.
///
/// Args:
///     text: Input text string
///     window_size: Size of the rolling window
///     base: Hash base (default: DEFAULT_BASE)
///     modulus: Hash modulus (default: DEFAULT_MOD)
///
/// Returns:
///     List of (hash_value, position) tuples for each window
#[pyfunction]
#[pyo3(signature = (text, window_size, base=None, modulus=None))]
fn rolling_hash_py(
    text: &str,
    window_size: usize,
    base: Option<u64>,
    ...
    let base = base.unwrap_or(DEFAULT_BASE.into());
    let modulus = modulus.unwrap_or(DEFAULT_MOD_U64);
    type T = u64;
    let rolling_hasher: RollingHash<'_, u8, T> =
        RollingHash::new(text.as_bytes(), window_size, base.into(), modulus.into()).unwrap();
    let results: Vec<(u64, usize)> = rolling_hasher.map(|(x, y)| (x as u64, y)).collect();
    Ok(results)
}

#[pymodule]
fn rolling_hash_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(rolling_hash_py, m)?)?;
    m.add("DEFAULT_MOD", DEFAULT_MOD_U64)?;
    Ok(())
}
